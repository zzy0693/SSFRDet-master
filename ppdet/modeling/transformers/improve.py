...
class TransformerLayer(nn.Layer):
    ...
    def forward(self, src, src_mask=None, pos_embed=None):
        ...
        src_ch = self.self_attn(q, k, value=src, attn_mask=src_mask)

        q_k= self.with_pos_embed(src1, pos_embed)
        q_k = q_k.transpose([0, 2, 1]).reshape(
            [-1, 256, h, w])  # 还原维度
        q_k = self.DWConv(q_k)
        q1 = k1 = q_k.flatten(2).transpose(
                [0, 2, 1])  # flatten [B, C, H, W] to [B, HxW, C]并且转成序列
        src1 = self.DWConv(src11)
        src1= src1.flatten(2).transpose(
                [0, 2, 1])  # flatten [B, C, H, W] to [B, HxW, C]并且转成序列
        src_pos = self.self_attn(q1, k1, value=src1, attn_mask=src_mask)
        src=src_ch+src_pos
...
class HybridEncoder(nn.Layer):
    ...
    def __init__(self,
                 ...
                 encoder_layer='TransformerLayer11',
    ...
    def forward(self, feats, for_mot=False):
        ...
        proj_feats = [self.input_proj[i](feat) for i, feat in enumerate(feats)]#重塑通道
        inputs=proj_feats
        # encoder
        if self.num_encoder_layers > 0:#插入AIFI
            for i, enc_ind in enumerate(self.use_encoder_idx):
                h, w = proj_feats[enc_ind].shape[2:] #提取H,W维度
                ...
                memory = self.encoder[i](src_flatten, pos_embed=pos_embed) #加自注意力
                proj_feats[enc_ind] = memory.transpose([0, 2, 1]).reshape(
                    [-1, self.hidden_dim, h, w])#还原维度

        # top-down fpn
        inner_outs = [inputs[-1]] #3层赋值
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            inner_out = self.fpn_blocks[len(self.in_channels) - 1 - idx](
                paddle.concat(
                    [upsample_feat, feat_low], axis=1))#fusion
        ...
        for idx in range(len(self.in_channels) - 1):
            ...
            if idx==1:
                feat_height = proj_feats[enc_ind]
            downsample_feat = self.downsample_convs[idx](feat_low)#蓝色模块
            out = self.pan_blocks[idx](paddle.concat(
                [downsample_feat, feat_height], axis=1))#fusion
            outs.append(out)#生成编码器输出
