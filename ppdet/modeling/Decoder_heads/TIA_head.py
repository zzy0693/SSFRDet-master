...
class TIAHead(nn.Layer):
    ...
    def __init__(self,
        ...
        self.SSFR_cls = nn.Conv2D(
            self.feat_channels, self.num_classes, 3, padding=1)
        self.SSFR_reg = nn.Conv2D(self.feat_channels, 4, 3, padding=1)
    ...
    def _init_weights(self):
        ...
        normal_(self.SSFR_cls.weight, std=0.01)
        constant_(self.SSFR_cls.bias, bias_cls)
        normal_(self.SSFR_reg.weight, std=0.01)
    ...
    def forward(self, feats):
        ...
        for feat, scale_reg, anchor_centers, stride in zip(
            ...
            cls_logits = self.SSFR_cls(cls_feat)
            if self.use_align_head:
            ...
            reg_dist = scale_reg(self.SSFR_reg(reg_feat).exp())
